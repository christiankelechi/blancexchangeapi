...
class Deposit( models.Model):
    ...
    def __str__(self):
        return self.profile.user.username + '-' + str(self.id)[:3]
    ...
    def update_deposits(cls, profile):
        ...
        for dep in deposits_data:
            address_match = True #addresses.filter(wallet_address=dep['wallet_address']).exists()

            if address_match:
                deposit, created = cls.objects.get_or_create(
                    transaction_id=dep['transaction_id'], 
                    defaults={'amount': dep['amount'], 'profile': profile, 'address': dep['address'], 'status': Deposit.Status.PENDING}
                )
                if created:
                    dep['address'] = dep['address'].address
